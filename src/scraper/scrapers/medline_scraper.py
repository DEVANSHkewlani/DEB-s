"""
MedlinePlus Scraper — Aggregated consumer health content
...
class MedlineScraper(BaseScraper):
    ...
    def run(self):
        ...
        self.logger.info("Starting MedlinePlus Scraping...")
        ...
        def process_one(disease):
            ...
            try:
                ...
                sections = self.extract_sections_from_soup(main_content)
                ...
                for header, text in sections.items():
                    ...
                    self.db.upsert_guideline({
                        ...
                        "title": f"MedlinePlus {content_type.replace('_', ' ').title()} for {disease}",
                ...
                self.logger.warning("Error scraping MedlinePlus for %s: %s", disease, e)
                failed += 1
