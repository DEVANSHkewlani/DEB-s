"""
NHS UK Scraper — Consumer-friendly plain-language health content
Uses change detection, section-based extraction with content_type metadata.
"""
from datetime import datetime
from concurrent.futures import ThreadPoolExecutor
from base_scraper import BaseScraper
from config import URLS, TARGET_DISEASES, MAX_THREADS
...
class NHSScraper(BaseScraper):
    ...
    def run(self):
        ...
        self.logger.info("Starting NHS Scraping...")
        ...
        def process_one(disease):
            ...
            try:
                ...
                content = (
                    ...
                    soup.find("div", class_="nhsuk-main-wrapper") or
                    soup.find("main")
                )
                if not content:
                    failed += 1
                ...
                for header, text in sections.items():
                    ...
                    self.db.upsert_guideline({
                        ...
                        "title": f"NHS {content_type.replace('_', ' ').title()} for {disease}",
                ...
                self.logger.warning("Error scraping NHS for %s: %s", disease, e)
                failed += 1
